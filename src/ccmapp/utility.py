...
def send_mobile_vcode_msg(phoneNumber, vcode, type='SMS_105935023'):
...
def send_mobile_alert_msg(phoneNumber, prjName, content, type='SMS_112455004'):
    METHOD_NAME = 'send_mobile_alert_msg'
    req = top.api.AlibabaAliqinFcSmsNumSendRequest()
    req.set_app_info(top.appinfo('24656980','e87c693c4588919338164c256112f1a4'))
    ...
    req.sms_type = "normal"
    req.sms_free_sign_name = "智慧建造平台"
    req.sms_param = "{name:'" + prjName + "',code:'"  + content + "'}"
    req.rec_num = phoneNumber
    req.sms_template_code = type
    try :
        resp = req.getResponse()
        logger.info("%s -- Got response: %s" % (METHOD_NAME,str(resp)))
        if resp["alibaba_aliqin_fc_sms_num_send_response"]["result"]["success"]:
            return True
        else:
            sys.stderr.write("%s" % str(resp))
            return False
    except Exception,e:
        raise e
        # return False
